...
if __name__ == '__main__':
    ...
    # Get the project root directory (assuming the script is in doctranslate/exporter/md)
    project_root = Path(__file__).parent.parent.parent.parent
    
    # Use a file from the examples directory for testing
    example_file = project_root / "examples" / "2206.01062v1.md"
    output_file = project_root / "output" / f"{example_file.stem}_exported.html"
    
    print(f"Using example file: {example_file}")
    print(f"Output will be saved to: {output_file}")

    d = MarkdownDocument.from_path(example_file)
    exporter = MD2HTMLExporter()
    d1 = exporter.export(d)
    
    # Ensure the output directory exists
    output_file.parent.mkdir(exist_ok=True)
    output_file.write_bytes(d1.content)
