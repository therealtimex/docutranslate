...
from doctranslate.agents.glossary_agent import GlossaryAgentConfig
...
from doctranslate.workflow.base import Workflow
...
from doctranslate.workflow.interfaces import DocxExportable, EpubExportable
...
async def lifespan(app: FastAPI):
    ...
    lang = (os.getenv("doctranslate_LANG") or "en").lower()
    if lang == "zh":
...
app = FastAPI(
    ...
    description=f"""
doctranslate 后端服务 API，提供文档翻译、状态查询、结果下载等功能。
...
async def main_page(request: Request):
    ...
    lang = (lang_q or os.getenv("doctranslate_LANG") or "en").lower()
    if lang == "en":
        # Default document language and title
        content = content.replace('lang="zh-CN"', 'lang="en"')
        content = content.replace(
            ">doctranslate - 交互式文档翻译<",
            ">doctranslate - Interactive Document Translation<",
        )
...
def run_app(port: int | None = None):
    ...
    load_env_file(None)
    initial_port = port or int(os.environ.get("doctranslate_PORT", 8010))
    lang = (os.getenv("doctranslate_LANG") or "en").lower()
    try:
        ...
        if lang == "zh":
            print(f"正在启动 doctranslate WebUI 版本号：{__version__}")
        else:
            print(f"Starting doctranslate WebUI version: {__version__}")
